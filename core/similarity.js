// similarity.js
//
// calculateSimilarity() is a method to compute the similarity score 
// based on the given distance_formula (euclidean, pearson)
//
...
const similarity = (items, person1, person2, distanceFormula) => {
  ...
  const scores = results.map((item) => {
    const score1 = items[person2][item]
    const score2 = items[person1][item]
